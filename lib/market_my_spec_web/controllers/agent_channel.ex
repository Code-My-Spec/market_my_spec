defmodule MarketMySpecWeb.AgentChannel do
  ...
  Inbound:
    * `reddit_result` — forwarded to whichever process is awaiting
      the matching `request_id` on Dispatcher.

  Outbound (from Endpoint.broadcast):
    * `reddit_fetch` — every binary on the topic receives this;
  ...
  @impl true
  def handle_in("reddit_result", %{"request_id" => req_id} = payload, socket) do
