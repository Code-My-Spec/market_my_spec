defmodule MarketMySpecWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: MarketMySpecWeb.Gettext
  ...
  attr :options, :list, doc: "the options to pass to PhoenixForm.options_for_select/2"
    ...
    <div class="fieldset mb-2">
      <label for={@id}>
        ...
        >
          ...
          {PhoenixForm.options_for_select(@options, @value)}
        </select>
      </label>
      <.error :for={msg <- @errors}>{msg}</.error>
    </div>
    ...
    <div class="fieldset mb-2">
      <label for={@id}>
        ...
        <textarea
          ...
          {@rest}
        >{PhoenixForm.normalize_value("textarea", @value)}</textarea>
      </label>
      <.error :for={msg <- @errors}>{msg}</.error>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(MarketMySpecWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(MarketMySpecWeb.Gettext, "errors", msg, opts)
    end
  end
