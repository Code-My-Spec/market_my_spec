defmodule MarketMySpecSpex.Story675.Criterion5724Spex do
  ...
  spex "invoking an unknown skill returns a clear MCP error" do
    scenario "agent calling invoke_skill with an unregistered skill name receives an error", context do
      ...
      when_ "the agent calls invoke_skill with an unknown skill name", context do
        mcp_conn =
          context.conn
          ...
          |> post(
            "/mcp",
            ~s({"jsonrpc":"2.0","method":"tools/call","id":1,"params":{"name":"invoke_skill","arguments":{"skill_name":"nonexistent-skill"}}})
          )

        {:ok, Map.put(context, :mcp_conn, mcp_conn)}
      end

      then_ "the MCP response contains a JSON-RPC error", context do
