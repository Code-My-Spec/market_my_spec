defmodule MarketMySpecSpex.Fixtures do
  ...
  **Rule of thumb:** Fixtures expose state that originates server-side
  ...
  # by exercising the pairing LiveView at `/app/agents/pair`. Reaching for
  ...
  defdelegate expect_reddit_fetch_envelope(timeout \\ 2_000),
    to: MarketMySpec.AgentsTestHelpers

  defdelegate receive_reddit_fetch_envelope(timeout), to: MarketMySpec.AgentsTestHelpers

  defdelegate respond_to_envelope(envelope, status, body),
    to: MarketMySpec.AgentsTestHelpers
  ...
  defdelegate dispatch_reddit(user, request, opts \\ []), to: MarketMySpec.Agents.Dispatcher
